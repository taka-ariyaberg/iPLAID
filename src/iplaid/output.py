...
def build_compound_and_dmso_rows(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Build separate compound and DMSO dispense rows.
    
    Separates compound dispenses from DMSO top-up dispenses for protocol generation.
    
    Args:
        df: DataFrame with Volume [uL], dmso_topup_uL, and Liquid Name columns
        
    Returns:
        Tuple of (compound_rows, dmso_rows, all_rows)
    """
    ...
    # Construct DMSO liquid name dynamically from actual DMSO stock if available
    # Otherwise use the compound metadata
    if len(dmso_rows) > 0:
        dmso_stock_str = df.loc[df["cmpdname"].str.upper() == "DMSO", "stock_conc_mM"].iloc[0] if any(df["cmpdname"].str.upper() == "DMSO") else 0.0
        dmso_rows["Liquid Name"] = f"[DMSO][{dmso_stock_str}]"
    else:
        dmso_rows["Liquid Name"] = "[DMSO][0.0]"  # Fallback for empty case
    ...
    return compound_rows, dmso_rows, all_rows
...
def build_liquid_table(
    ...
    Creates a mapping of unique liquids to source plate wells with proper
    sorting (DMSO first, then compounds alphabetically).
...
def attach_and_sort_dispense_rows(
    ...
    all_rows = all_rows.merge(
        liquid_table[["Liquid Name", "compound", "stock_mM", "is_dmso"]],
        on="Liquid Name",
        how="left"
    )

    all_rows = all_rows.sort_values(
        by=["is_dmso", "compound", "stock_mM", "Volume [uL]", "Source Well", "Target Plate", "Target Well"],
        kind="mergesort"
    ).reset_index(drop=True)
